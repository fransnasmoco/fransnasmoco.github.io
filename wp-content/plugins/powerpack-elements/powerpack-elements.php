 ...
 * Plugin URI: https://MadxArtwork.net
 * Description: Custom addons for madxartwork page builder.
 * Version: 1.3.9.1
 * Author: MadxArtwork - PowerPack Elements
 * Author URI: http://MadxArtwork.net
...
define( 'POWERPACK_ELEMENTS_VER', '1.3.9.1' );
...
define( 'POWERPACK_ELEMENTS_madxartwork_VERSION_REQUIRED', '1.7' );
...
/**
 * Check if madxartwork is installed
 *
 * @since 1.0
 *
 */
...
/**
 * Shows notice to user if madxartwork plugin
...
function pa_fail_load() {
	...
	if ( _is_madxartwork_installed() ) {
        ...
        $message = __( 'PowerPack requires madxartwork plugin to be active. Please activate madxartwork to continue.', 'powerpack' );
		$button_text = __( 'Activate madxartwork', 'powerpack' );
		...
		$activation_url = wp_nonce_url( self_admin_url( 'update.php?action=install-plugin&plugin=madxartwork' ), 'install-plugin_madxartwork' );
        $message = sprintf( __( 'PowerPack requires %1$s"madxartwork"%2$s plugin to be installed and activated. Please install madxartwork to continue.', 'powerpack' ), '<strong>', '</strong>' );
		$button_text = __( 'Install madxartwork', 'powerpack' );
	}
 ...
 * madxartwork version if outdated
 *
 * @since 1.0
 *
 */
function pa_fail_load_out_of_date() {
	...
	$message = __( 'PowerPack requires madxartwork version at least ' . POWERPACK_ELEMENTS_madxartwork_VERSION_REQUIRED . '. Please update madxartwork to continue.', 'powerpack' );
...
function pp_init() {
    ...
    // Notice if the madxartwork is not active
	...
	if ( ! version_compare( madxartwork_VERSION, POWERPACK_ELEMENTS_madxartwork_VERSION_REQUIRED, '>=' ) ) {
...
{
	$settings = get_site_option( 'pp_madxartwork_settings' );
	...
	update_site_option( 'pp_madxartwork_settings', $settings );
}
