<?php
// Get all madxartwork page templates
if ( !function_exists('pp_get_page_templates') ) {
    function pp_get_page_templates( $type = '' ) {
        ...
        if ( $type ) {
            $args['tax_query'] = [
                [
                    'taxonomy' => 'madxartwork_library_type',
                    'field'    => 'slug',
                    'terms' => $type,
                ]
            ];
...
{
    $enabled_modules = \PowerpackElements\Classes\PP_Admin_Settings::get_option( 'pp_madxartwork_modules', true );
