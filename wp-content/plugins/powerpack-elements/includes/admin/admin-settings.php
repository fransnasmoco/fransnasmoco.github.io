...
<div class="wrap">
    ...
    <form method="post" id="pp-settings-form" action="<?php echo self::get_form_action( '&tab=' . $current_tab ); ?>">
        ...
        <h2 class="nav-tab-wrapper pp-nav-tab-wrapper">
                ...
                <a href="<?php echo self::get_form_action( '&tab=general' ); ?>" class="nav-tab<?php echo ( $current_tab == 'general' ? ' nav-tab-active' : '' ); ?>"><?php esc_html_e( 'General', 'powerpack' ); ?></a>
                <?php if ( 'off' == $settings['hide_wl_settings'] ) { ?>
                    <a href="<?php echo self::get_form_action( '&tab=white-label' ); ?>" class="nav-tab<?php echo ( $current_tab == 'white-label' ? ' nav-tab-active' : '' ); ?>"><?php esc_html_e( 'White Label', 'powerpack' ); ?></a>
                <?php } ?>
                <a href="<?php echo self::get_form_action( '&tab=modules' ); ?>" class="nav-tab<?php echo ( $current_tab == 'modules' ? ' nav-tab-active' : '' ); ?>"><?php esc_html_e( 'Elements', 'powerpack' ); ?></a>
        </h2>
    ...
    <p>
        <?php
            ...
            $support_link = !empty( $support_link ) ? $support_link : 'https://MadxArtwork.net/contact/';
